...
namespace RyansLibrary.Labyrinth
{
    public class BlueprintParserTests
    {
        ...
        public void SetUp()
        {
            ...
            ShapeData shape1x2x1 = MakeShape();
            ...
            ShapeData shape2x1x2 = MakeShape();
        ...
        */
        ...
        #region Parser Rotation / Cell State Tests
        /// <summary>
        /// A 2x1x1 (cells along X) over two blueprints along Z only fits turned. With CanRotate off the
        /// parser only makes unturned candidates, so nothing fits. With it on, exactly the two turned fits
        /// come back: 270 anchored on (0,0,0), and 90 anchored on (1,0,0).
        /// </summary>
        [Test]
        public void TestParserOnlyTurnsShapesThatCanRotate()
        {
            // Arrange
            Vector3Int o = RandomVector();
            Blueprint start = AddBlueprint(o);
            ...
            ShapeData fixedLong = MakeShape(Vector3Int.zero, Vector3Int.right);
            ShapeData turningLong = MakeRotatableShape(Vector3Int.zero, Vector3Int.right);

            // Act
            List<ShapeCandidate> fixedShapes = _parser.CheckValidShapes(start, new List<ShapeData> { fixedLong });
            List<ShapeCandidate> turnedShapes = _parser.CheckValidShapes(start, new List<ShapeData> { turningLong });

            // Assert
            Assert.AreEqual(0, fixedShapes.Count);
            Assert.AreEqual(2, turnedShapes.Count);
            Assert.IsTrue(turnedShapes.Exists(c => c.Anchor == Vector3Int.zero && c.Rotation == RoomRotation.Deg270));
            Assert.IsTrue(turnedShapes.Exists(c => c.Anchor == Vector3Int.right && c.Rotation == RoomRotation.Deg90));
        }

        /// <summary>
        /// A NeedBlueprint cell has to sit on a blueprint for the shape to fit, but it belongs to a
        /// neighbouring room, not this one. The shape fills from its single Blueprint cell and that is
        /// the only cell it covers; the needed blueprint is left for another room.
        /// </summary>
        [Test]
        public void TestParserNeedBlueprintCellIsNotCovered()
        {
            // Arrange
            Vector3Int o = RandomVector();
            Blueprint start = AddBlueprint(o);
            ...
            ShapeData needsRight = MakeShapeStates(
                (Vector3Int.zero, CellState.Blueprint),
                (Vector3Int.right, CellState.NeedBlueprint));

            // Act
            List<ShapeCandidate> validShapes = _parser.CheckValidShapes(start, new List<ShapeData> { needsRight });

            // Assert
            Assert.AreEqual(1, validShapes.Count);
            ShapeCandidate only = validShapes[0];
            Assert.AreEqual(Vector3Int.zero, only.Anchor);
            CollectionAssert.AreEquivalent(new[] { Vector3Int.zero }, only.CoveredCells);
        }
        #endregion
        ...
        /// <summary>
        /// CheckConfigs only accepts a checked cell marked Blueprint. Every other state is a neighbour
        /// constraint, not part of the room, so the check fails even though a blueprint sits on the cell
        /// and all six neighbours are DontCare.
        ///
        /// This guards ParseBlueprints, which calls CheckConfigs with arbitrary offsets via
        /// candidate.Anchor + localPosition and can land on a constraint cell.
        /// </summary>
        [TestCase(CellState.NoBlueprint)]
        [TestCase(CellState.DontCare)]
        [TestCase(CellState.NeedBlueprint)]
        public void TestConfigRejectsNonBlueprintCell(CellState state)
        {
            // Arrange
            Vector3Int o = RandomVector();
            Blueprint b1 = AddBlueprint(o);

            ShapeData shape = MakeShapeStates((Vector3Int.zero, state));
        ...
        /// <summary>
        /// A NeedBlueprint neighbour must have a blueprint in it, same as a Blueprint neighbour. Fails
        /// with nothing to the right, passes once a blueprint is placed there.
        /// </summary>
        [Test]
        public void TestConfigNeedBlueprintRequiresNeighbor()
        {
            // Arrange
            Vector3Int o = RandomVector();
            Blueprint b1 = AddBlueprint(o);
            ...
            // Act / Assert
            Assert.IsFalse(_parser.CheckConfigs(Vector3Int.zero, needsRight, b1));      // nothing to the right yet
            AddBlueprint(o + Vector3Int.right);
            Assert.IsTrue(_parser.CheckConfigs(Vector3Int.zero, needsRight, b1));
        }
        ...
        /// <summary>
        /// Builds a ShapeData whose listed cells are all CellState.Blueprint. Rotation is off so every
        /// anchor cell makes exactly one candidate; use MakeRotatableShape to test turned shapes.
        /// </summary>
        private ShapeData MakeShape(params Vector3Int[] blueprintCells)
        {
            ShapeData shape = ScriptableObject.CreateInstance<ShapeData>();
            SetPrivateField(shape, "_cells", new Dictionary<Vector3Int, CellState>());     // Cells has no setter and starts null
            SetPrivateField(shape, "_canRotate", false);
        ...
        /// <summary>
        /// Same as MakeShape, but the parser also tries the shape turned 90, 180 and 270 degrees.
        /// </summary>
        private ShapeData MakeRotatableShape(params Vector3Int[] blueprintCells)
        {
            ShapeData shape = MakeShape(blueprintCells);
            SetPrivateField(shape, "_canRotate", true);
            return shape;
        }

        /// <summary>
        /// Builds a ShapeData with an explicit CellState per cell, for shapes that declare negative
        /// space. Any position not listed is implicitly DontCare. Rotation is off, as in MakeShape.
        /// </summary>
        private ShapeData MakeShapeStates(params (Vector3Int cell, CellState state)[] cells)
        {
            ShapeData shape = MakeShape();

...
namespace RyansLibrary
{
    public class BlueprintParserTests
    {
        ...
        /// <summary>
        /// CheckForValidCells is deliberately permissive where CheckForValidOrigins is not: it hands
        /// back every Blueprint-state cell as a potential anchor without testing whether that anchor
        /// actually fits. Same blueprint, same shape, two different answers - two cells vs null.
        /// This is the seam where CheckValidShapes trades early pruning for correctness, so pin it.
        /// </summary>
        [Test]
        public void TestValidCellsIgnoresFitWhileValidOriginsDoesNot()
        {
            // Arrange
            Vector3Int o = RandomVector();
            Blueprint b1 = AddBlueprint(o);         // lone blueprint, a 2x1x1 cannot possibly fit

            // Act
            List<Vector3Int> cells = _parser.CheckForValidCells(b1, _shapes[1]);
            List<Vector3Int> origins = _parser.CheckForValidOrigins(b1, _shapes[1]);

            // Assert
            Assert.AreEqual(2, cells.Count);        // both cells offered as anchors regardless of fit
            Assert.Contains(Vector3Int.zero, cells);
            Assert.Contains(Vector3Int.right, cells);
            Assert.IsNull(origins);                 // ...but neither anchor is actually valid
        }
